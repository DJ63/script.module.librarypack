...
def parse_dom(html, name='', attrs=None, req=False, exclude_comments=False):
    ...
    if isinstance(html, six.text_type) or isinstance(html, DomMatch):
        ...
        try:
            ...
            logger.log("HTML Decode Failed. Data length: %d" % len(html), xbmc.LOGWARNING)
            try:
                html = [html.decode("utf-8", "replace")]
            except:
                logger.log("HTML Decode Failed (Replace). Data length: %d" % len(html), xbmc.LOGWARNING)
                html = [html]
    elif not isinstance(html, list):
        logger.log("Input isn't list or string/unicode.", xbmc.LOGWARNING)
        return ''

    if not name:
        logger.log("Missing tag name", xbmc.LOGWARNING)
        return ''

    if not isinstance(attrs, dict):
        logger.log("Attrs must be dictionary", xbmc.LOGWARNING)
        return ''
