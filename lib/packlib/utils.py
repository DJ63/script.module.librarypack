...
def make_list_item(label, meta, art=None, cast=None):
    ...
    try:
        ...
        pass
    if 'ids' in meta and 'imdb' in meta['ids']:
        listitem.setProperty('imdb_id', str(meta['ids']['imdb']))
    if 'ids' in meta and 'tvdb' in meta['ids']:
...
def _byteify(data, ignore_dicts=False):
    ...
    if isinstance(data, dict) and not ignore_dicts:
        return dict(
            [(_byteify(key, ignore_dicts=True), _byteify(value, ignore_dicts=True)) for key, value in iteritems(data)])
    return data
    ...
    try:
        ...
        with kodi.ProgressDialog(kodi.get_name(), i18n('downloading') % (file_name), background=background, active=active) as pd:
            try:
                ...
                for key in headers: headers[key] = unquote(headers[key])
            except:
                headers = {}
            ...
            logger.log('Downloading: %s -> %s' % (url, full_path), xbmc.LOGDEBUG)
            ...
            try:
                ...
                logger.log('Path Create Failed: %s (%s)' % (e, path), xbmc.LOGDEBUG)
            ...
            while True:
                ...
                logger.log('Position : %s / %s = %s%%' % (total_len, content_length, percent_progress), xbmc.LOGDEBUG)
        ...
        if not cancel:
            ...
            logger.log('Download Complete: %s -> %s' % (url, full_path), xbmc.LOGDEBUG)

    except Exception as e:
        logger.log('Error (%s) during download: %s -> %s' % (str(e), url, file_name), xbmc.LOGERROR)
...
def url2name(url):
    url = url.split('|')[0]
    return os.path.basename(unquote(urlsplit(url)[2]))
...
def choose_list(Trakt_API, translations, username=None):
    i18n = translations.i18n
    trakt_api = Trakt_API(kodi.get_setting('trakt_oauth_token'), kodi.get_setting('use_https') == 'true',
