...
from kodi_six import xbmcaddon, xbmcplugin, xbmcgui, xbmc
import sys
import os
import re
import json
import time
from packlib import CustomProgressDialog
import six
    ...
    class MetaClass(type):
        ...
        def __str__(self):
            ...
            return '|{0}| -> |{1}|{2}|{3}|{4}|{5}|'.format(
                self.version, self.major, self.minor, self.tag, self.tag_version, self.revision
            )
...
def kodiVersion():
    ...
    if version >= 11.0 and version <= 11.9:
        ...
        codename = 'Krypton'
    elif version >= 18.0 and version <= 18.1:
        codename = 'Leia'
    else:
        codename = "Decline"
...
def sortX(string):
    try:
        ...
        string = re.sub("""REPL[0-f][0-f]""", '', str(string))
        return string
    except:
        return string
...
def countGitHubIssues(url):
    try:
        ...
        r = dom_parser2.parse_dom(c, 'div',
                                  {'class': re.compile('table-list-header-toggle\sstates\sfloat-left\spl-\d+')})
